...
export class SheetsSyncService {
  ...
  static async updateUserStatsWithPagination(
    ...
    try {
      ...
      const results = [];
      const unmatchedPlayers = new Map(); // Track unmatched players
      ...
      for (const [weekNum, weekData] of weekGroups.entries()) {
        try {
            ...
            try {
              ...
              for (const player of userTeam.players) {
                ...
                if (playerPerformance) {
                  ...
                  record.count++;
                  if (record.users.length < 10) { // Limit to 10 for readability
          ...
          // Update weekly rankings for the processed week
      ...
      return {
        success: true,
        results,
        unmatchedPlayers: Array.from(unmatchedPlayers.values()),
        hasMoreUsers,
