...
static async updateWeeklyRankings(tournamentId, weekNumber) {
  try {
    ...
    // Get all weekly stats for this tournament and specific week
...
static async updateOverallRankings(tournamentId) {
  try {
    ...
    for (const teamDoc of teamsSnapshot.docs) {
      ...
      const referralPoints = userDoc.exists() ? (userDoc.data().referralPoints || 0) : 0;
      
      // Total points is sum of weekly points and referral points
      const finalTotalPoints = totalPoints + referralPoints;
